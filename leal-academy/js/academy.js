if(typeof academyCourses === "undefined"){
    window.academyCourses = [];
}

if(typeof academyMaterials === "undefined"){
    window.academyMaterials = {};
}
const state = {
...
async function ensureAcademyCoursesLoaded(){
    let loadedRemote = window.academyDataSource === "supabase";

    if(!loadedRemote && typeof loadAcademyFromSupabase === "function"){
        try{
            loadedRemote = await loadAcademyFromSupabase();
        }
        catch(error){
            console.warn(
...
function removeUnavailableLibrasLesson(course){
    ...
    applyCorrectLibrasVisuals(course);
    return course;
}
